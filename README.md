...
*비용 배수는 세 케이스에서 거의 동일(high 3.1~3.2× · pro 2.0×)하고, 시간 배수만 케이스에 따라 변동(high 15~20×) — 위 표는 그 평균입니다.*
...
> 뫽타주 윗줄 = 입력(편집 시) + `gpt-image-2` quality 변형(low→medium→high), 아랫줄 = Gemini 모델.
>
...
| 변형                       | 비용          | 소요시간     | 토큰    | 품질(관찰)                |
| ------------------------ | ----------- | -------- | ----- | --------------------- |
| `gpt-image-2` (low)      | **$0.0083** | 16.7s    | 509   | 책 제거 + 인물 재구성(장면 재해석) |
